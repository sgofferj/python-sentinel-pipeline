...
def osint_ramp_colormap(x: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    if HAS_CUDA:
        x_g = cp.array(x)
        r = cp.where(x_g < 0.5, 0, (x_g - 0.5) * 2 * 255)
        g = cp.where(x_g < 0.5, 200 + (x_g * 2) * 55, 255 - (x_g - 0.5) * 2 * 255)
        b = cp.where(x_g < 0.5, x_g * 2 * 255, 255 - (x_g - 0.5) * 2 * 255)
        res = (
            cp.asnumpy(r).astype(np.uint8),
            cp.asnumpy(g).astype(np.uint8),
            cp.asnumpy(b).astype(np.uint8)
        )
        del x_g, r, g, b
        return res

    r = np.where(x < 0.5, 0, (x - 0.5) * 2 * 255)
    g = np.where(x < 0.5, 200 + (x * 2) * 55, 255 - (x - 0.5) * 2 * 255)
    b = np.where(x < 0.5, x * 2 * 255, 255 - (x - 0.5) * 2 * 255)
        ...
        with rio.open(out_path, 'w', **profile) as dst:
            ...
            for _, window in dst.block_windows(1):
                ...
                if np.any(target_mask):
                    ...
                    s2_win_data[0][target_mask] = np.clip(tr * 0.4 + s2_win_data[0][target_mask] * 0.6, 0, 255).astype(np.uint8)
                    s2_win_data[1][target_mask] = np.clip(tg * 0.4 + s2_win_data[1][target_mask] * 0.6, 0, 255).astype(np.uint8)
                    s2_win_data[2][target_mask] = np.clip(tb * 0.4 + s2_win_data[2][target_mask] * 0.6, 0, 255).astype(np.uint8)

                mask_norm = (s2_win_data[3].astype(float) / 255.0) * (win_geom_mask.astype(float) / 255.0)
                s2_win_data[3] = (mask_norm * 255).astype(np.uint8)
                for b in range(3):
                    s2_win_data[b] = (s2_win_data[b].astype(float) * mask_norm).astype(np.uint8)
                dst.write(s2_win_data, window=window)

        func.perf_logger.end_step()
        build_overviews_gdal(out_path)
        meta.generate_sidecar(out_path, "FUSED-RADAR-BURN", "RADAR-BURN")
        ...
        with rio.open(out_path, 'w', **profile) as dst:
            for _, window in dst.block_windows(1):
                ...
                if HAS_CUDA:
                    ...
                    vh_db = cp.asnumpy(vh_db_g); ndbi_clean = cp.asnumpy(ndbi_clean_g)
                    vh_gate = cp.asnumpy(vh_gate_g); x = cp.asnumpy(x_g)
                    ...
                    vh_gate = vh_db > -15
                    x = np.clip((ndbi_clean + 0.6) / 0.8, 0, 1)
                    x[vh_gate] = np.clip(x[vh_gate] * 1.4, 0, 1)

                tr, tg, tb = osint_ramp_colormap(x)
                ...
                final_alpha_norm = (tci_data[3].astype(float) / 255.0) * (win_geom_mask.astype(float) / 255.0)

                for b, col in enumerate([tr, tg, tb]):
                    tci_data[b] = (col.astype(float) * ghost_mask + tci_data[b].astype(float) * (1 - ghost_mask)).astype(np.uint8)

                tci_data[3] = (final_alpha_norm * 255).astype(np.uint8)
                for b in range(3):
                    tci_data[b] = (tci_data[b].astype(float) * final_alpha_norm).astype(np.uint8)
                dst.write(tci_data, window=window)

        func.perf_logger.end_step()
        build_overviews_gdal(out_path)
        meta.generate_sidecar(out_path, "FUSED-TARGET-PROBE-V2", "TARGET-PROBE-V2")
        cog.convert_to_cog(out_path)
    ...
    with rio.open(tci_path) as tci_src, rio.open(vh_path) as vh_src, rio.open(nirfc_path) as nirfc_src:
        ...
        with rio.open(out_path, 'w', **profile) as dst:
            for _, window in dst.block_windows(1):
                ...
                context_blue = (context_blue.astype(float) * alpha_norm).astype(np.uint8)
                dst.write(np.stack([vh_boosted, ndvi_scaled, context_blue, alpha_final], axis=0), window=window)

        func.perf_logger.end_step()
        build_overviews_gdal(out_path)
        meta.generate_sidecar(out_path, "FUSED-LIFE-MACHINE", "LIFE-MACHINE")
        cog.convert_to_cog(out_path)
...
def run_correlation() -> None:
    ...
    for match in matches:
        ...
        fuse_life_machine(vh_ana, tci_vis, nirfc_vis, s2_name, match['inter_geom'])
        fuse_target_probe_v2(vh_ana, ndbi_ana, ndre_ana, tci_vis, s2_name, match['inter_geom'])
