...
def rebuild_inventory():
    ...
    visual_root = os.path.join(c.BASE_DIR, "output/visual")
    inventory = {"layers": []}
            ...
            if file.endswith(".json") and file != "inventory.json":
                ...
                try:
                    with open(json_path, 'r', encoding='utf-8') as f:
                        ...
                        rel_path = os.path.relpath(json_path.replace(".json", ".tif"), c.DIRS['OUT'])
                        meta["path"] = rel_path
    ...
    inventory["layers"].sort(key=lambda x: x.get("acquisition_time", ""), reverse=True)

    inventory_path = os.path.join(c.DIRS['OUT'], "visual/inventory.json")
    with open(inventory_path, 'w', encoding='utf-8') as f:
        json.dump(inventory, f, indent=2)
    
    print(f"Global inventory updated: {len(inventory['layers'])} layers indexed.")
